...
type Body = tuple[float, float, TimePosition]
...
def newton_step(t1: float, x1: complex, v1: complex, a1: complex, dt: float, bodies: list[Body]) -> tuple[float, complex, complex, complex]:
    t2: float = t1 + dt
    a2: complex = sum(g(m, abs(x1 - p(t1))) for m, _, p in bodies)  # m/s^2
...
def newton_loop(t0: float, x0: complex, v0: complex, dt: float, t_count: int, bodies: list[Body]) -> tuple[list[float], list[complex], list[complex], list[complex]]:
    ...
    x: list[complex] = [x0]
    v: list[complex] = [v0]
    a: list[complex] = [sum(g(m, abs(x0 - p(t0))) for m, _, p in bodies)]
    for _ in range(t_count):
        t1: float = t[-1]
        x1: complex = x[-1]
        v1: complex = v[-1]
        a1: complex = a[-1]
        t2, x2, v2, a2 = newton_step(t1, x1, v1, a1, dt, bodies)
        t.append(t2)
        x.append(x2)
        v.append(v2)
        a.append(a2)
...
def newton_generator(t0: float, x0: complex, v0: complex, dt: float, bodies: list[Body]):
    ...
    yield x
    while True:
        t, x, v, a = newton_step(t, x, v, a, dt, bodies)
        yield x
